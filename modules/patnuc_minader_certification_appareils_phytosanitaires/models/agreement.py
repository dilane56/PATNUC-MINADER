...
class SeedOperatorAgreement(models.Model):
    ...
    """
    _name = 'phytosanitary.certification.agreement'
    _description = 'Agrément pour demandeur de certtification des appareil de traitements '
    ...
    operator_id = fields.Many2one('phytosanitary.certification.operator', string='Opérateur',required=True, tracking=True)
    ...
    def init(self):
        ...
        self.env.cr.execute(
            "CREATE INDEX IF NOT EXISTS idx_phyto_agreement_state_expiry "
            "ON phytosanitary_certification_agreement (state, expiry_date)"
        )
        # Index sur operator_id pour les mises à jour en batch
        self.env.cr.execute(
            "CREATE INDEX IF NOT EXISTS idx_phyto_agreement_operator "
            "ON phytosanitary_certification_agreement (operator_id)"
        )
    ...
    def create(self, vals):
        ...
        if vals.get('name', _('Nouveau')) == _('Nouveau'):
            vals['name'] = self.env['ir.sequence'].next_by_code('phytosanitary.certification.agreement') or _('Nouveau')
    ...
    def _cron_check_expiring_agreements(self):
        ...
        if expired_agreements:
            ...
            for agreement in expired_agreements:
                _logger.info(f"  -> {agreement.name} - Opérateur: {agreement.operator_id.partner_name}")
        else:
    ...
    def name_get(self):
        ...
        result = []
        for agreement in self:
            name = f"{agreement.name} - {agreement.operator_id.partner_name}"
            if agreement.ceas_number:
