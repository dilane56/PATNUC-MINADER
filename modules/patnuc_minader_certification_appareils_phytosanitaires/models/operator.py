...
class CertificationOperator(models.Model):
    _name = 'phytosanitary.certification.operator'
    _description = 'Opérateur habilité à effectuer une demande de certification des appareil '
    ...
    name = fields.Char('Référence ',
        ...
        readonly=True,
        copy=False,
        default=lambda self: self.env['ir.sequence'].next_by_code('phytosanitary.certification.operator')
    )
    # -------------------------
    ...
    # Relations métier
    certification_requests = fields.One2many('phytosanitary.certification.request', 'user_id', string='Demandes de certification')
    agreement_ids = fields.One2many('phytosanitary.certification.agreement', 'operator_id', string='Agréments')
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('phytosanitary.certification.operator') or 'New'
    ...
    def action_view_agreements(self):
        ...
        return {
            ...
            'res_model': 'phytosanitary.certification.agreement',
            'view_mode': 'tree,form',
