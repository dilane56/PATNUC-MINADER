...
class CertificationInspection(models.Model):
    ...
    inspected_area = fields.Float(string="Superficie/nombre de plants inspectée(s)", help="Surface en ha ou nombre de plants")
    ...
    support_structure = fields.Char(string="Structure d'encadrement ou d'appui", compute='_compute_parcelle_location', store=True)
    ...
    def action_done(self):
        for rec in self:
            ...
            if not rec.report_attachment and not rec.conclusion and not rec.estimated_yield:
                raise UserError(" Veillez Ajoutez une conclusion , un rapport et la recolte estimée avant de clore l'inspection.")
            rec.state = 'done'
