...
class FertilizerHomologation(models.Model):
    _name = 'fertilizer.homologation'
    _description = 'Procédure d\'homologation des engrais et fertilisants'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = 'create_date desc'

    name = fields.Char(string='Référence', required=True, copy=False, readonly=True, default=lambda self: _('New'))
    applicant_id = fields.Many2one('res.users', string='Demandeur', required=True, tracking=True)
    product_id = fields.Many2one('fertilizer.product', string='Produit', required=True, tracking=True)
    ...
    #reference de l'arrêté 

    decree_id = fields.Many2one(
        'fertilizer.decree',
        string="Arrêté d'Homologation Associé",
        readonly=True,
        copy=False,
    )
...
###################################### upload de fichier ###################################
    ...
    def create(self, vals):
        ...
        return super(FertilizerHomologation, self).create(vals)
    
    def write(self, vals):
        # Capturer les noms de fichiers depuis le contexte lors de la modification
        self._capture_filenames(vals)
        result = super(FertilizerHomologation, self).write(vals)
        ...
        for binary_field, filename_field in binary_fields.items():
            ...
            if binary_field in vals and vals[binary_field]:
                ...
                if filename_field not in vals or not vals[filename_field]:
                    ...
                    if not filename:
                        default_names = {
                            'official_request_letter': 'Lettre_de_demande_officielle',
                            'homologation_certificate': 'Attestation_d\'homologation.pdf',
                            'import_agreement_copy': 'Copie_agrément_d\'importation.pdf',
                            'identity_document_copy': 'Photocopie_pièce_d\'identité.pdf',
                        }
                        filename = default_names.get(binary_field, f'{binary_field}.pdf')
    ...
    def action_sign(self):
        for record in self:
            if record.state == 'approved':
                ...
                record.date_delivrance = fields.Datetime.today().now()
                record.state = 'signed'
                #create de l'arrete
                new_decree = self.env['fertilizer.decree'].create({
                    # Seuls les champs obligatoires et essentiels sont passés ici.
                    # L'homologation_id déclenchera le pré-remplissage dans le modèle Arrêté.
                    'homologation_id': record.id,
                    'commercial_name': record.product_id.id,
                    # Nom Technique du produit
                    'technical_name': record.product_id.technical_name,
                     # Fabricant du produit homologué
                    'manufacturer_id': record.product_id.manufacturer_id.id, 
                    'decree_file': record.homologation_document, # Bien que related, nous le passons si nécessaire.
                    'decree_filename': record.homologation_document_filename,
                    'delivery_date': record.date_delivrance, # La date de délivrance doit être une date simple.
                    'state': 'actif', # L'arrêté est immédiatement Actif
                    
                })

                # 2. Mise à jour de l'Homologation avec la référence de l'Arrêté
                record.decree_id = new_decree.id
                #reponse 
    ...
    def action_reject(self):
        return {
            ...
            'res_model': 'fertilizer.rejection.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
                'default_homologation_id': self.id,
    ...
    def _notify_applicant_rejection(self):
        ...
        if self.applicant_id:
            self.message_post(
                body=_("Votre demande d'homologation %s a été rejetée.") % self.name,
                partner_ids=[self.applicant_id.id],
                message_type='comment'
            )
    ...
    def action_return_to_draft(self):
        return {
            ...
            'res_model': 'fertilizer.return.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
                'default_homologation_id': self.id,
    ...
    def action_return_to_verification(self):
        return {
            'type': 'ir.actions.act_window',
            'name': "Retourner à la vérification",
            'res_model': 'fertilizer.return.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
                'default_homologation_id': self.id,
                'default_current_state': self.state,
                'default_target_state': 'verification'
            }
        }
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('fertilizer.homologation') or _('New')
        self._capture_filenames(vals)
        return super(FertilizerHomologation, self).create(vals)
    ...
    def unlink(self):
        ...
        return super(FertilizerHomologation, self).unlink()