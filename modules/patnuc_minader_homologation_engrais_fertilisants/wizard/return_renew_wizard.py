...
class FertilizerRenewReturnWizard(models.TransientModel):
    _name = 'fertilizer.renew.return.wizard'
    _description = 'Wizard de retour de demande de renouvellement d\'homologation'

    renew_homologation_id = fields.Many2one('fertilizer.renew.homologation', string='Demande de modification', required=True)
    ...
    def action_confirm_return(self):
        ...
        if self.target_state == 'draft' and self.homologation_id.create_uid:
            ...
            message = _("Votre demande de renouvellement d'homologation %s a été retournée au brouillon.\n\nMotif: %s") % (
                self.renew_homologation_id.name, self.return_reason
            )
            ...
            message = _("La demande de renouvellement d'homologation %s a été retournée à l'étape dossier reçu.\n\nMotif: %s") % (
                self.renew_homologation_id.name, self.return_reason
            )
            ...
            message = _("La demande de renouvellement d'homologation %s a été retournée à l'étape de vérification administrative.\n\nMotif: %s") % (
                self.renew_homologation_id.name, self.return_reason
            )
            # Notifier tous les utilisateurs du groupe agent_SRE
            sre_group = self.env.ref('patnuc_minader_homologation_engrais_fertilisants.group_agent_SRE', raise_if_not_found=False)
            ...
            message = _("Votre demande de renouvellement d'homologation %s a été retournée.\n\nMotif: %s") % (
                self.renew_homologation_id.name, self.return_reason
            )
            partner_ids = [self.renew_homologation_id.create_uid.partner_id.id] if self.renew_homologation_id.create_uid else []
        if partner_ids:
            self.renew_homologation_id.message_post(
                body=message,
