...
class FertilizerReturnWizard(models.TransientModel):
    _name = 'fertilizer.return.wizard'
    _description = 'Wizard de retour de demande d\'homologation'
    ...
    def action_confirm_return(self):
        ...
        if self.target_state == 'draft' and self.homologation_id.create_uid:
            ...
            message = _("Votre demande d'homologation %s a été retournée au brouillon.\n\nMotif: %s") % (
                self.homologation_id.name, self.return_reason
            )
            ...
            message = _("La demande d'homologation %s a été retournée à l'étape dossier reçu.\n\nMotif: %s") % (
                self.homologation_id.name, self.return_reason
            )
            ...
            message = _("La demande d'homologation %s a été retournée à l'étape de vérification administrative.\n\nMotif: %s") % (
                self.homologation_id.name, self.return_reason
            )
            # Notifier tous les utilisateurs du groupe agent_SRE
            sre_group = self.env.ref('patnuc_minader_homologation_engrais_fertilisants.group_agent_SRE', raise_if_not_found=False)
            ...
            partner_ids = [self.homologation_id.create_uid.partner_id.id] if self.homologation_id.create_uid else []
        if partner_ids:
            self.homologation_id.message_post(
                body=message,
